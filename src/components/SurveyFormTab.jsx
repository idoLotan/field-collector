...
import { loadSurveyDraft, persistSurveyDraft, clearSurveyDraft, loadAddrPairs, getSurveyNextId } from '../utils/storage';

/* ── SVG icons (reused from FormTab) ── */
...
export default function SurveyFormTab({ active, onSaved, showToast, openLightbox }) {
  const draft = useMemo(() => loadSurveyDraft(), []);
  ...
  const [residents, setResidents]           = useState(draft?.residents || '');
  const [ageGroup, setAgeGroup]             = useState(draft?.ageGroup || '');
  const [tenancy, setTenancy]               = useState(draft?.tenancy || '');
  ...
  useEffect(() => {
    persistSurveyDraft({
      ...
      propertyType, residents, ageGroup, tenancy, condition,
      ownerId, ownerName,
      notes, photos: stagingPhotos,
    });
  }, [addrMode, selectedStreet, selectedHouse, freeText, capturedLat, capturedLon,
      propertyType, residents, ageGroup, tenancy, condition, notes, stagingPhotos]);
  ...
  const saveRecord = () => {
    ...
    const record = {
      ...
      lon: capturedLon || '',
      propertyType, residents, ageGroup, tenancy, condition,
      ownerId: ownerId.trim(), ownerName: ownerName.trim(),
      notes: notes.trim(),
    ...
    setPropertyType(''); setResidents(''); setAgeGroup(''); setTenancy(''); setCondition('');
  ...
  const catDone    = !!(propertyType || residents || ageGroup || tenancy || condition);
  const ownerDone  = !!(ownerId || ownerName);
    ...
    <>
      ...
      <div className="form-tiles">
        ...
        <button className={`form-tile${catDone ? ' done' : ''}`} onClick={() => setStep('category')}>
          ...
          <span className="form-tile-label">סיווג נכס</span>
        ...
        <button className={`form-tile${ownerDone ? ' done' : ''}`} onClick={() => setStep('owner')}>
          <div className="form-tile-icon"><IconPerson /></div>
          <span className="form-tile-label">בעל/ת הדירה</span>
          {ownerDone && <span className="form-tile-check">✓</span>}
        </button>
  ...
  const categoryScreen = (
    <>
      <div className="step-header">
        <h2>סיווג נכס</h2>
        <p>בחר את המאפיינים הרלוונטיים</p>
      </div>

      <ChipRow label="סוג נכס" value={propertyType} onChange={setPropertyType}
        options={['דירה', 'בית פרטי', 'עסק', 'מוסד']} />

      <ChipRow label="נפשות משוערות" value={residents} onChange={setResidents}
        options={['1', '2-3', '4-5', '6+', 'לא ידוע']} />

      <ChipRow label="קבוצת גיל דומיננטית" value={ageGroup} onChange={setAgeGroup}
        options={['ילדים', 'צעירים', 'משפחות', 'קשישים', 'מעורב']} />

      <ChipRow label="דיירות" value={tenancy} onChange={setTenancy}
        options={['בעלים', 'שוכרים', 'לא ידוע']} />

      <ChipRow label="מצב נכס" value={condition} onChange={setCondition}
        options={['תקין', 'טעון טיפול', 'הרוס/נטוש']} />
  ...
  const ownerScreen = (
    <>
      <div className="step-header">
        <h2>בעל/ת הדירה</h2>
        <p>חפש ברשימה שהועלתה או הזן ידנית</p>
      </div>
      <div className="field">
        <OwnerField
          ownerId={ownerId}
          ownerName={ownerName}
          onChange={(id, name) => { setOwnerId(id); setOwnerName(name); }}
          showToast={showToast}
        />
      </div>
      <button className="btn-skip" onClick={() => { setOwnerId(''); setOwnerName(''); setStep(null); }}>
        ללא פרטי בעל דירה
      </button>
      <div className="step-footer">
        <Back />
  ...
  const notesScreen = (
    <>
      ...
      <div className="field">
        <NotesField key={`snotes-${formKey}`} notes={notes} onChange={setNotes} showToast={showToast} />
      </div>
  ...
  const photosScreen = (
    <>
      ...
      <PhotosField key={`sphotos-${formKey}`} photos={stagingPhotos}
